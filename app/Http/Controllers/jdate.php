...
/*	F	*/
function jdate($format, $ago = '', $timestamp = '', $none='', $time_zone = 'Asia/Tehran', $tr_num='en'){
    ...
    for($i=0; $i<$sl; $i++){
        ...
        switch($sub){
            ...
            case'c':
                $out.=$j_y.'/'.$j_m.'/'.$j_d.'_'.$date[0].':'.$date[1].':'.$date[6];
                break;
...
function gregorian_to_jalaliAPI($gy,$gm,$gd,$mod=''){
    list($gy,$gm,$gd)=explode('_',tr_num($gy.'_'.$gm.'_'.$gd));/* <= Extra :اين سطر ، جزء تابع اصلي نيست */
    $g_d_m=array(0,31,59,90,120,151,181,212,243,273,304,334);
    if($gy > 1600){
        $jy=979;
    ...
    return $jd . ' ' . jdate_words(array('mm'=>$jm),' ') . ' ' . $jy;
}
